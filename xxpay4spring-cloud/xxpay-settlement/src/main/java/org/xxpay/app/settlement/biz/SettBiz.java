 ...
 */
package org.xxpay.app.settlement.biz;
...
import org.xxpay.common.enumm.PublicEnum;
...
public class SettBiz {
	...
	public void launchDailySettCollect(List<RpAccount> accountList, Date endDate) {
		...
		for (RpAccount rpAccount : accountList) {
			try {
				LOG.debug(rpAccount.getUser_no()+ ":开始汇总");
				dailySettCollectBiz.dailySettCollect(rpAccount, endDate);
				LOG.debug(rpAccount.getUser_no() + ":汇总结束");
			} catch (Exception e) {
				LOG.error(rpAccount.getUser_no()+":汇总异常", e);
			}
		}
	}
